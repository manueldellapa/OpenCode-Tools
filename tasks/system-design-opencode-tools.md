...
Per l'adapter 1.17.18, stdout deve essere interamente NDJSON. Ogni linea deve avere schema/event type ammesso, lo stesso `sessionID` e limiti dimensionali; una warning non JSON, inclusa quella di fallback, fa fallire il trasporto. Gli eventi `text` completati vengono raggruppati per `part.messageID` e ordinati secondo la sequenza osservata; reasoning e tool events non entrano nel messaggio di protocollo. La terminalità si deriva dalla struttura di lifecycle `step_finish` verificata ogni volta che lo stream ne espone almeno un evento (issue #85): è terminale soltanto il gruppo completato e non vuoto il cui `messageID` coincide con l'ultimo evento `step_finish` osservato prima della chiusura dello stream -- e quello `step_finish` deve essere davvero l'ultima parola dello stream. Un `text` completato il cui valore è vuoto dopo `.strip()` non costituisce mai prova di una risposta terminale; se è l'ultimo completamento per un `messageID`, invalida anche un eventuale candidato precedente dello stesso messaggio (issue #87). Qualunque evento di message-lifecycle riconosciuto (`step_start`, `tool_use`, `reasoning`, `error` o `text`, completato o meno) osservato dopo l'ultimo `step_finish` lo squalifica: la conclusione di quell'attività resta non attestata, quindi né il `step_finish` precedente né il testo a cui punterebbe possono essere considerati la fine dello stream, anche quando quel testo è l'unico mai completato. Un nuovo `step_finish` successivo ristabilisce un confine provvisoriamente fidato. Inoltre, il `reason` canonico OpenCode 1.17.18 `"tool-calls"` identifica una conclusione non terminale: se è il reason dell'ultimo `step_finish` a EOF, il decoder fallisce al livello transport con `opencode.transport_incomplete_tool_call_lifecycle`; lo stesso reason su uno step intermedio resta valido quando un lifecycle successivo termina realmente (per esempio con `reason="stop"`). Soltanto quando lo stream non espone alcun evento `step_finish` un singolo gruppo completato e non vuoto è considerato terminale senza ulteriori verifiche; in assenza di tale evento, più di un gruppo completato resta equivoco. Poiché il `messageID` di `step_finish` è determinante per la terminalità, un evento `step_finish` con `messageID` assente, nullo, vuoto o di tipo diverso da stringa non viene mai ignorato come se non portasse informazione di lifecycle: fallisce chiuso allo stesso modo di una `text` malformata, non solo quando esistono più candidati. La stessa scrutinio si applica al `part.type` di `step_finish`: deve valere esattamente `"step-finish"`, il valore reale 1.17.18, allo stesso modo in cui una `text` richiede `part.type == "text"`; un `part.type` assente, di tipo sbagliato o pari a un altro valore di lifecycle (ad esempio `"step-start"`) fallisce chiuso invece di essere trattato come se l'evento non portasse informazione di lifecycle. Testo terminale mancante, lifecycle tool-call incompleto o candidati rimasti equivoci sono `PROTOCOL_ERROR`.
